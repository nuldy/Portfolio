...
const ThemeManager = {
    ...
    applyTheme(theme) {
        ...
        if (metaThemeColor) {
            metaThemeColor.setAttribute('content', theme === 'dark' ? '#111111' : '#A67D3B');
        }
    },
     ...
     */
    toggle() {
        const newTheme = this.theme === this.themes.light 
            ? this.themes.dark 
            : this.themes.light;
...
const ParallaxManager = {
     ...
     */
    init() {
        this.elements = document.querySelectorAll('.hero-image-decoration, .hero-gradient');
